...
2. **Continuous scroll, guided focus.** All nine steps render in one column. The progress strip both *shows where you are* and *jumps you there*. The active step gets a soft ring; other steps stay visible but de-emphasized.
...
5. **Sticky operational chrome.** Top header, patient bar, progress strip, and footer are sticky/fixed. The scrollable area is the feed only.
6. **AI is an assistant, not a step.** Long-term direction (not yet shipped): hide Step 5 from the linear feed; expose AI through the persistent header chip and the right-side drawer. The validator already treats Step 5 as optional.
...
| `ConsultationWorkspaceShell` | 3-column grid (left rail · scrollable feed · right rail/drawer). |
| `ConsultationProgressStrip` | Sticky progress + autosave status + advisory missing list. |
| `ConsultationContinuousFeed` | Render the 9 step blocks in order with `data-workflow-step`. |
...
## 4. Recommended next iteration (not yet shipped)

Tracked here so the next pass has a focused checklist.

1. **Hoist recording controls.** Keep the persistent chip in the top header; remove the duplicate control row inside `Step05AI` (let Step 5 be view-only with the status pill + retry).
...
4. **Autosave label.** Suppress the `Syncing…` flash by entering the saving state only after the user has stopped typing for >300 ms.
...
- Start a new consultation → progress strip sticks, every step scroll-aligns under it.
- Click each step from the strip → exactly one smooth scroll; ring visibly lands on the targeted card.
- Press `Alt+→` / `Alt+←` while not focused in an input → step changes with the same scroll behaviour.
- Type rapidly in the Notes step → no layout shift; autosave label transitions `Syncing… → Synced` once per debounce window.
