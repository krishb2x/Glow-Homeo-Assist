  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20 px-4">
      ...
      {activeTab === "config" && (
        ...
        {/* Product Level overrides - Full Width and Spacious Layout */}
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden w-full">
          <div className="p-8 border-b border-slate-150 bg-slate-50/20">
            <h3 className="font-extrabold text-slate-900 text-base">Product Level overrides</h3>
            <p className="text-sm text-slate-400 mt-0.5">Override discounts and partner commissions per product type.</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm border-collapse">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-500">
                <tr>
                  <th className="px-8 py-5.5 font-bold text-xs uppercase tracking-widest">Product / Category</th>
                  <th className="px-8 py-5.5 font-bold text-xs uppercase tracking-widest w-56">User Discount</th>
                  <th className="px-8 py-5.5 font-bold text-xs uppercase tracking-widest w-56">Partner Commission</th>
                  <th className="px-8 py-5.5 font-bold text-xs uppercase tracking-widest text-center w-32">Status</th>
                </tr>
              </thead>
                ...
                {configs.map((cfg, index) => (
                  ...
                  >
                    <td className="px-8 py-5.5">
                      <div className="font-black text-slate-800 text-base">{cfg.name}</div>
                      <div className="text-xs text-slate-400 font-extrabold uppercase tracking-widest mt-1">{cfg.product_type.replace('_', ' ')}</div>
                      <div className="text-xs text-slate-500 font-bold mt-2 inline-block bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200">Base Price: ₹{cfg.price}</div>
                    </td>
                    
                    <td className="px-8 py-5.5">
                      <div className="flex rounded-xl overflow-hidden border border-slate-200 focus-within:border-emerald-600 focus-within:ring-2 focus-within:ring-emerald-600 transition-all bg-slate-50 shadow-inner h-11 max-w-[180px]">
                        <input 
                          type="number"
                          min="0"
                          ...
                          className="w-full px-4 py-2.5 text-sm font-bold focus:outline-none bg-transparent"
                        />
                        <select 
                          value={cfg.discount_type}
                          disabled={!cfg.is_active}
                          onChange={e => handleConfigChange(index, 'discount_type', e.target.value as any)}
                          className="border-l border-slate-200 bg-slate-100 text-xs font-black px-3.5 focus:outline-none cursor-pointer"
                        >
