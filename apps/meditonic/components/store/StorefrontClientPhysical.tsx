...
import { Search, X, SlidersHorizontal, BookOpen, ChevronDown, Check, ArrowUpDown } from "lucide-react";
...
export default function StorefrontClientPhysical({ initialCatalog, categories }: StorefrontClientProps) {
  ...
  const processedCatalog = useMemo(() => {
    ...
    if (selectedFormats.length > 0) {
      result = result.filter(item => {
        ...
        const isPhysical = format.includes('hardcover') || format.includes('paperback') || format.includes('physical') || format.includes('hardcopy');
        const isEbook = !isCombo && !isPhysical;

        return (
          (selectedFormats.includes('combo') && isCombo) ||
          (selectedFormats.includes('physical') && isPhysical) ||
          (selectedFormats.includes('ebook') && isEbook)
        );
      });
    }
    ...
    <div className="space-y-7">
      ...
      <div>
        ...
        <div className="space-y-2.5">
          {[
            { id: 'physical', label: '📘 Hard Copy' },
            { id: 'ebook', label: '📱 eBook (PDF)' },
    ...
    <div className="w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between w-full max-w-7xl mx-auto pb-4 border-b border-slate-100">
        ...
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            type="text"
            placeholder="Search medical books, diagnostic guides..."
            value={searchQuery}
